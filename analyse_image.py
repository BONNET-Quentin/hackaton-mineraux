...
def simulation_calcul():
    ...
    np.save('ims.npy', ims)  # Sauvegarde des images pour une utilisation ultérieure
    np.save('T-C.npy', [T, C])  # Sauvegarde des matrices T et C
    return ("Calcul 3D terminé")




def affichage(T, C, ims, direction, plan):
    ...
    ani = generate_animation(ims, direction, plan, interval=50, fig=fig, ax=ax1)
...
# Utilisation de l'analyse 2D

# ICI CHOISIR DE SIMULER
#simulation_calcul()  
T,C = np.load('T-C.npy', allow_pickle=True)  # Charger les matrices T et C
ims = np.load('ims.npy', allow_pickle=True)  # Charger les images pré-générées

méthode = 1 # 0 pour plan aléatoire, 1 pour plan choisi
...
else: # ATTENTION 0 PRENDRE UN PLAN QUI EXISTE DANS LA MATRICE
