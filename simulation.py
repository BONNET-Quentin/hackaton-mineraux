...
def update_mat(T, f, f_param):
    ...
    # au dessus
    Tt = np.roll(T, 1, 0)
    Tt[[0,1, -1,-2], :, :] = 1; Tt[:, [0,1, -1,-2], :] = 1; Tt[:, :, [0,1, -1,-2]] = 1 #padding
    ...
    # en dessous
    Tbo = np.roll(T, -1, 0)
    Tbo[[0,1, -1,-2], :, :] = 1; Tbo[:, [0,1, -1,-2], :] = 1; Tbo[:, :, [0,1, -1,-2]] = 1
    ...
    # à gauche
    Tl = np.roll(T, 1, 1)
    Tl[[0,1, -1,-2], :, :] = 1; Tl[:, [0,1, -1,-2], :] = 1; Tl[:, :, [0,1, -1,-2]] = 1
    ...
    # à droite
    Tr = np.roll(T, -1, 1)
    Tr[[0,1, -1,-2], :, :] = 1; Tr[:, [0,1, -1,-2], :] = 1; Tr[:, :, [0,1, -1,-2]] = 1
    ...
    # devant
    Tf = np.roll(T, 1, 2)
    Tf[[0,1, -1,-2], :, :] = 1; Tf[:, [0,1, -1,-2], :] = 1; Tf[:, :, [0,1, -1,-2]] = 1
    ...
    # derrière
    Tba = np.roll(T, -1, 2)
    Tba[[0,1, -1,-2], :, :] = 1; Tba[:, [0,1, -1,-2], :] = 1; Tba[:, :, [0,1, -1,-2]] = 1
